...
#
# Devices are two /tmp/invfs-e2e image files. NOTE: blkio treats /dev/* paths as
# raw devices, so the script cd's into /tmp/invfs-e2e and uses RELATIVE image
...
import os, random
random.seed(2525)
d = "/tmp/invfs-e2e/wp25multi/orig"
...
import sys
rp = int(sys.argv[1])
d0 = open("/tmp/invfs-e2e/wp25md0.img","rb").read()
d1 = open("/tmp/invfs-e2e/wp25md1.img","rb").read()
...
(
    ...
    [ "${TA:-0}" -ge 1 ] || fail "A has no live copies"
    echo "A promoted: $TA copies live"
    ...
    [ "${LIVE:-0}" -ge 1 ] || { cat "$WORK/e-sweepB.log"; fail "demotion evicted the hot copies too"; }
    echo "pressure demoted $DEM cold copies (A's); $LIVE hot copies (B's) live"
