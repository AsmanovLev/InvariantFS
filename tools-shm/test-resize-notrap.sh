...
# Uses /tmp/invfs-e2e (tmpfs) like the other soak scripts. NOTE: blkio treats
# /dev/* paths as raw devices, so the script cd's into /tmp/invfs-e2e and uses
...
B=$REPO/bin
WORK=/tmp/invfs-e2e/wp18resize
#trap 'rm -rf "$WORK" /tmp/invfs-e2e/wp18r-*.img' EXIT  # e2e hygiene: /tmp/invfs-e2e is a small tmpfs
...
PROBE_EOF
gcc -std=gnu11 -O2 -I$REPO/src -I$REPO/src/core -I$REPO/src/codecs -I$REPO/src/recipes -I$REPO/src/vendor7z -o "$WORK/tools/rszprobe" \
...
random.seed(18)
d = "/tmp/invfs-e2e/wp18resize/orig"
...
python3 -c "
import os
open('/tmp/invfs-e2e/wp18resize/post.bin','wb').write(os.urandom($POST_BLOCKS * 4096))"
...
python3 -c "
import os
open('/tmp/invfs-e2e/wp18resize/big.bin','wb').write(os.urandom(300 * 1024 * 1024))"
...
d = "/tmp/invfs-e2e/wp18resize/origb"
