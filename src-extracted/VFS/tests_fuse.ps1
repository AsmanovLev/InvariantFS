...
$r = wsl -d void -- bash -c "cd /mnt/d/VFS && gcc -O2 -o build_linux/invf-fuse src/cli/fuse_fs.c src/core/volume.c src/core/vol_cpack.c src/core/vol_png.c src/core/vol_seal.c src/core/vol_repair.c src/core/vol_rollback.c src/core/vol_resize.c src/core/vol_fsck.c src/core/vol_crash.c src/core/vol_exer.c src/core/vol_dedupe.c src/core/vol_textzone.c src/core/vol_heat.c src/core/vol_sweep.c src/core/vol_read.c src/core/vol_write.c src/core/vol_records.c src/core/vol_ast.c src/core/vol_dirs.c src/core/arc.c src/core/crc32c.c src/codecs/lz4.c src/recipes/flacx.c src/recipes/tarx.c src/recipes/pngx.c src/codecs/miniz.c -lfuse3 -Wl,-l:libzstd.so.1 -lz -Isrc -Isrc/core -Isrc/codecs -Isrc/recipes -Isrc/vendor7z -pthread -DINVFS_EMBED_FLACX -DMINIZ_NO_ZLIB_APIS 2>&1 && echo BUILD-OK"
