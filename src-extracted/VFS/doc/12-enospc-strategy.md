...
> **Статус:** разделы «Watermarks + Throttling», «Резервирование для root
> (5%)», «Размер RAW Zone при форматировании» и «Поведение приложения»
> (EAGAIN/EDQUOT) — **первоначальный дизайн, не реализован** (в коде нет
> ни вотермарков, ни троттлинга, ни квот, ни пресетов mkfs; READONLY —
> односторонний лэтч). Как есть на самом деле — раздел «Реализация
> (2025-08)» внизу: резерв + hard-min + READONLY, атомарный ENOSPC,
> RAW→SHADOW spill. Плюс WP16b: файлы, не прошедшие по цене места на
> sweep'е, получают класс DEFER_ENOSPC и ждут в RAW (06-sweep-worker.md).
...
- **Адаптивное усилие сжатия в RAW (WP23)**: у лестницы вотермарков
  ...
  sweep → throttle → spill RAW→SHADOW → READONLY на hard_min**.
