...
| PB3 | **FIXED** (`ae0f4a9a` + `acadac72`, verified 2026-08-30): FUSE `.destroy` calls `vol_close` (flush + journal compact + sb CLEAN) with a `main()` fallback, and `vol_open` auto-recovers a DIRTY volume when the scan is anomaly-free. A clean unmount mounts CLEAN; a kill -9 mounts via replay (`test-writepath.sh` leg D3 exercises exactly this) | fuse_fs.c invf_destroy/main; volume.c vol_open | A4/A5 |
| PB4 | **FIXED (2026-08-30, WP4ab)**: `invf_fsync` now commits the write session and calls `vol_sync` = `vol_flush` + `blkio_flush` (a real `fsync(2)` barrier on the image; WP21 crash leg proves the fsynced region survives kill -9). Per-op write-through/group-commit for non-fsync traffic remains WP3 | blkio.c:608-616; volume.c vol_sync; fuse_fs.c invf_fsync | A3/A4 |
| PB5 | **FIXED (2026-08-30, WP4b)**: FUSE write() streams through engine sessions (`vol_write_begin/range/commit`): per-handle memory is O(1) (one staged 64K tail segment; 300MB stream grew daemon RSS ~13MB, not ~300MB), complete segments compress+store immediately, ranged pwrite with sub-segment RMW and sparse gaps, swept files materialize to RAW on first write, commit = new record then old retire (vol_replace_file ordering). `vol_replace_file` stays for the sweep/offline callers | fuse_fs.c wctx/invf_write/commit_wctx; volume.c invfs_wsession | A3 |
...
| H4 | **FIXED** (`ae0f4a9a`, kept by the WP4b session commit): FUSE flush/fsync propagate late ENOSPC as `-ENOSPC`; a failed commit aborts the session and rolls the volume back to the pre-session state instead of dropping the error | fuse_fs.c commit_wctx | A4 |
...
| WP4 | **DONE** (landed earlier: `1cf65a2f` engine + WP17 tuning; acceptance-verified 2026-08-30). Streaming write path: per-handle O(1) (one staged 64K tail segment, rest streams via vol_write_begin/range sessions); ranged pwrite at any offset (sub-segment RMW, sparse gaps); mmap read of ALL storage forms + mmap write via FUSE_CAP_WRITEBACK_CACHE (writeback flows through .write); gpg --verify acceptance leg passes. Suite: tools/test-writepath.sh (legs A-F). Followup fix this session: vol_open now flock(LOCK_EX\|LOCK_NB)s the image on POSIX — a lingering post-unmount daemon drain + offline tool pair used to corrupt the volume (two live writers); test mnt_down waits for daemon exit | code-fix | L (long pole) |
...
| WP21 | **DONE** Sweep-checkpoint + rollback. CKP0 descriptor at block-0 offset **0x220** (the spec's suggested 0x180 lies inside RSZ0's 0x140..0x20C payload -- moved to the first free 0x20 boundary): {inode_area_pos, journal_pos, stage_pba/blocks, sweep_seq, time}, magic+crc32c, RMW like RDP0. invf-sweep arms it BEFORE the walk (vol_ckp_begin), having **staged the used journal prefix** -- the spec's bare "journal end position" is NOT sufficient against this code: vol_flush rewrites the journal in place from l2p_dirty, so the pre-sweep L2P survives only as that RSZ0-style staged copy (the inode area is truly append-only and needs none). While armed, vol_free_blocks never frees: blocks stay bitmap-allocated and are marked in a per-run retmap; vol_ckp_end writes them as the hidden **\x01reten** owner (seal-owner pattern: AST+L2P, sharded, length field carries BLOCKS -- the record is never served). Sweep end-state: CKP0 live + retained held; fsck counts them live, verify --deep skips the 0x01 owner. **invf-rollback** (new CLI, tools/invf-rollback.c -- house-separate like invf-resize, not an fsck flag): verifies CKP0 + stage (per-entry replay CRC), restores the journal, zero-guards the inode area at the checkpoint (the commit point), re-runs the fsck rebuild (reused, not reimplemented) which keeps retained blocks (referenced by the resurrected records) and reclaims all post-sweep allocations incl. the stage; CKP0 cleared LAST (crash-safe: phases idempotent, INVFS_ROLLBACK_ABORT_AT=restored/rebuilt legs in the e2e). --realize deletes the registry (the retire path frees exactly the mapped ranges) + clears CKP0 (point of no return); a bare sweep auto-realizes the previous checkpoint first (the K=1 contract kept suite-compatible). Refusals: no checkpoint on read-only/recovering volumes, under a live seal (parity pins absolute pbas), with INVFS_CHECKPOINT=0, or with no free run for the stage; rollback refuses under a live seal (rc 2) and on a corrupt descriptor/stage (rc 3, post-sweep state untouched). Deviations from the spec text, all in retention's favor or forced by code reality: (a) retention engages per checkpoint-armed sweep SESSION regardless of any free_on_delete-style knob (the spec's own "REGARDLESS" line) and post-sweep deletes free immediately (documented); (b) rollback never reads the registry -- truncation + journal restore make it vanish and the rebuild reclaims its blocks, so "free its blocks explicitly" is realized as orphan reclaim; (c) a sweep that retained nothing leaves NO checkpoint (disarm: identity rollback is pointless, quiescent re-sweeps stay quiescent); (d) the seal's membership now excludes retention-registry blocks (seal_view.is_ret) so realize under a live seal does not churn stripes -- without it test-seal's idempotent re-seal leg fails; (e) invf-fsck -f REFUSES while CKP0 is live (its orphan reclaim is the one pass that could free a not-yet-registered retained block; report mode unaffected, and no existing suite used -f); (f) an applied resize clears CKP0 (vol_rsz0_apply commit) -- resize.c untouched per the constraints; the registry survives verbatim and the next sweep's defensive realize reclaims it; (g) vol_sweep_pending (daemon) defers while CKP0 is live -- the on-demand path has no registry of its own. e2e: tools/test-rollback.sh (7 legs: bit-exact resurrection incl. classes gone, post-checkpoint writes discarded, realize PONR, retention fidelity (deleted-after-sweep file resurrects), crash legs at restored/rebuilt + crash-mid-sweep via INVFS_SWEEP_ABORT_AFTER, double-rollback/fresh-volume/sealed refusals) | code | M |
