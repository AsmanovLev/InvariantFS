...
> **⚠ ФОРМАТ В АКТИВНОЙ РАЗРАБОТКЕ.**
> Этот документ описывает **текущее** состояние, а не стабильную спецификацию.
> Meta-v3 (COW B+ tree base + append-only Delta Log) меняется; v2-механизмы
> (`CKP0`/`\x01reten`, L2P-журнал) выведены из обращения, но часть их кода и
> тестов ещё лежит в дереве. **Не рассчитывайте на совместимость томов,
> созданных разными сборками**, и не используйте этот документ как контракт
> для сторонних реализаций. Обязательные заголовки разделов ниже описывают
> намерение, а не зафиксированное поведение.
>
> Known open issues: `impl_docs/old_docs/AUDIT.md`, `INCIDENTS.md`.
