...
Правило горизонта отката: при живом CKP0 оба входа компакции
(`invf-sweep --compact` и авто-проход в конце sweep'а) громко отказывают
(`inode compact: skipped (sweep checkpoint … live)`) — сначала разрешите
чекпоинт. Живой seal не мешает: записи `\x01parity*` переживают срез
побайтно, inode-область вне полос parity (чекпоинт под seal не армится).
